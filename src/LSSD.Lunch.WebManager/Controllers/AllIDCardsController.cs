...
namespace LSSD.Lunch.WebManager.Controllers
{
    [Route("[controller]")]
    [ApiController]
    public class AllIDCardsController : ControllerBase
    {
        private readonly ILogger<AllIDCardsController> _logger;
        private readonly StudentService _service;

        public AllIDCardsController(ILogger<AllIDCardsController> logger, StudentService service)
        {
            _logger = logger;
            _service = service;
        }
        ...
        {            
            ...
            byte[] fileBytes = null;
            string downloadFilename = "lunchcards-all-" + DateTime.Today.ToLongDateString().Replace(" ", "").Replace("-","").Replace(",","") + ".docx";
